...
/**
 * This class handles the additionnal sip features of a converged session It is a delegate since it is used by many http session
 * implementations classes (Standard and clustered ones)
 *
 *
 * @author Jean Deruelle
 * @author Vladimir Ralev
 *
 *         This class is based on the contents of org.mobicents.servlet.sip.catalina.session.ConvergedSessionDelegate class from
...
public class ConvergedSessionDelegate {
    ...
    public String encodeURL(String relativePath, String scheme) {
        ...
        // ListenersService
        List<UndertowListener> webServerListeners = context.getWebServerListeners();
        // Host
        ...
        while (i < webServerListeners.size() && port < 0) {
            UndertowListener listener = webServerListeners.get(i);
            if (scheme != null && ("http".equalsIgnoreCase(scheme) || "https".equalsIgnoreCase(scheme))
                    && listener instanceof HttpListenerService) {
                if (((HttpListenerService) listener).getBinding().get() != null) {
                    port = ((HttpListenerService) listener).getBinding().get().getAbsolutePort();
                    if (((HttpListenerService) listener).getBinding().get().getAddress() != null) {
                        hostname = ((HttpListenerService) listener).getBinding().get().getAddress().getHostName();
                    }
                }
            }
            i++;
    ...
    public MobicentsSipApplicationSession getApplicationSession(boolean create) {
        ...
        if (key != null) {
            ...
            if (sipAppSession != null) {
                // make sure to acquire this app session and add it to the set of app sessions we monitor in the context of the
                // application
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(key + " found in http session " + httpSession.getId()
                            + " but not present in the manager. already invalidated ? creating new sip application session");
                }
            }
        }
        ...
        MobicentsSipApplicationSession sipApplicationSession = sipManager.findSipApplicationSession(httpSession);
        if (sipApplicationSession == null && create) {
            ...
            SipApplicationSessionKey sipApplicationSessionKey = SessionManagerUtil
                    .getSipApplicationSessionKey(sipContext.getApplicationName(), null, null);

            sipApplicationSession = sipManager.getSipApplicationSession(sipApplicationSessionKey, true);

            // Store the App Session ID in the HTTP sessions to recover it from there when it's transfered to a new node.
            httpSession.setAttribute(APPLICATION_SESSION_ID_ATTRIBUTE_NAME, sipApplicationSession.getKey());
