...
public class ConvergedServletInitialHandler extends ServletInitialHandler {
    ...
    private static final Logger log = LoggerFactory.getLogger(ConvergedServletInitialHandler.class);

    private final ConvergedServletContextImpl convergedServletContext;

    public ConvergedServletInitialHandler(ServletPathMatches paths, HttpHandler next, Deployment deployment, ServletContextImpl servletContext) {
        super(paths, next, deployment, servletContext);
        this.convergedServletContext = new ConvergedServletContextImpl(servletContext);
    }
    ...
    private void dispatchRequest(final HttpServerExchange exchange, final ServletRequestContext servletRequestContext, final ServletChain servletChain, final DispatcherType dispatcherType) throws Exception {
        ...
        if (dispatcherType == DispatcherType.REQUEST || dispatcherType == DispatcherType.ASYNC) {
            log.warn("dispatcherType value is DispatcherType.REQUEST or DispatcherType.ASYNC");
        } else {
