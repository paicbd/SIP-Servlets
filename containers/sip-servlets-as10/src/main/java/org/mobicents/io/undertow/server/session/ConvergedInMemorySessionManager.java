 ...
 *
 */
public class ConvergedInMemorySessionManager implements SessionManager, SessionManagerStatistics{
    ...
    public ConvergedInMemorySessionManager(String deploymentName, int maxSessions, boolean statisticsEnabled) {
    ...
    private static class ConvergedSessionImpl implements Session {
        ...
        void invalidate(final HttpServerExchange exchange, SessionListener.SessionDestroyedReason reason) {
            ...
            sessionManager.handleSessionInvalidationForStatistics(creationTime);
    ...
    protected void handleSessionInvalidationForStatistics(long sessionCreationTime) {
        if(statisticsEnabled) {
            long avg, newAvg;
            do {
                avg = averageSessionLifetime.get();
                BigDecimal bd = new BigDecimal(avg);
                bd.multiply(new BigDecimal(expiredSessionCount.get())).add(bd);
                newAvg = bd.divide(new BigDecimal(expiredSessionCount.get() + 1), MathContext.DECIMAL64).longValue();
            } while (!averageSessionLifetime.compareAndSet(avg, newAvg));


            expiredSessionCount.incrementAndGet();
            long life = System.currentTimeMillis() - sessionCreationTime;
            long existing = longestSessionLifetime.get();
            while (life > existing) {
                if (longestSessionLifetime.compareAndSet(existing, life)) {
                    break;
                }
                existing = longestSessionLifetime.get();
            }
        }
    }
