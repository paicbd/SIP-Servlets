...
import org.mobicents.as10.Constants;
...
public class SIPWebContext extends SipContextImpl {
    ...
    public void start() throws ServletException {
        ...
        JBossWebMetaData mergedMetaData = warMetaData.getJBossWebMetaData();
        if (mergedMetaData instanceof JBossConvergedSipMetaData) {
        	logger.debugf("JBossConvergedSipMetaData has been already attached to the deploymentUnit");
        } else {
        	logger.debugf("No JBossConvergedSipMetaData has been already attached to the deploymentUnit");
        ...
        try {
            processMetaData(mergedMetaData);
        } catch (Exception e) {
    ...
    private void processMetaData(JBossWebMetaData mergedMetaData) throws Exception {
        // processJBossWebMetaData(sharedJBossWebMetaData);
        // processWebMetaData(sharedJBossWebMetaData);
    	//already called in augmentAnnotations
        //JBossSipMetaDataMerger.merge((JBossConvergedSipMetaData) mergedMetaData, null, sipMetaData);
