...
public class UndertowSipDeploymentProcessor implements DeploymentUnitProcessor {
	...
	// copied from "protected"
	// UndertowDeploymentProcessor.hostNameOfDeployment()
	public static String hostNameOfDeployment(final WarMetaData metaData, final String defaultHost) {
		...
		if (hostName == null) {
			throw ROOT_LOGGER.nullHostName();
		}
		return hostName;
	}

	// copied from "protected"
	// UndertowDeploymentProcessor.pathNameOfDeployment()
	public static String pathNameOfDeployment(final DeploymentUnit deploymentUnit, final JBossWebMetaData metaData) {
		...
		if (metaData.getContextRoot() == null) {
			...
			if (description != null) {
				// if there is an EEModuleDescription we need to take into
				// account that the module name may have been
				// overridden
	...
	public void deploy(DeploymentPhaseContext phaseContext) throws DeploymentUnitProcessingException {
		...
		if (!isSipDeployment(deploymentUnit)) {
			return;
		}

		addToConvergedDeployments(phaseContext.getServiceRegistry(), deploymentUnit.getName());
		...
		ServiceController<?> defaultServerServiceController = phaseContext.getServiceRegistry()
				.getService(defaultServerServiceName);
		...
		final ServiceName deploymentServiceName = UndertowService.deploymentServiceName(serverInstanceName, hostName,
				pathName);
		ServiceController<?> deploymentServiceController = phaseContext.getServiceRegistry()
				.getService(deploymentServiceName);
		...
		ServiceController<?> deploymentInfoServiceController = phaseContext.getServiceRegistry()
				.getService(deploymentInfoServiceName);
		...
		ServiceController<?> sipDeplyomentInfoServiceController = phaseContext.getServiceRegistry()
				.getService(sipDeploymentInfoServiceName);
		UndertowSipDeploymentInfoService sipDeplyomentInfoService = (UndertowSipDeploymentInfoService) sipDeplyomentInfoServiceController
				.getService();
		sipDeplyomentInfoService.getDeploymentInfoServiceInjectedValue()
	...
	public void undeploy(DeploymentUnit context) {
		if (isSipDeployment(context)) {
			removeFromConvergedDeployments(context.getServiceRegistry(), context.getName());
		}
	}

	private void addToConvergedDeployments(ServiceRegistry serviceRegistry, String deploymentName) {
		...
		containerService.addConvergedDeployment(deploymentName);
	}
	...
	private boolean isSipDeployment(DeploymentUnit deploymentUnit) {
		...
		if (sipMetaData == null) {
			...
			if (sipAnnotationMetaData == null || !sipAnnotationMetaData.isSipApplicationAnnotationPresent()) {
				...
				// When no sip.xml but annotations only, Application is not
				// recognized as SIP App by AS7

				// In case there is no metadata attached, it means this is not a
				// sip deployment, so we can safely
				// ignore
				// it!
				return false;
			}
