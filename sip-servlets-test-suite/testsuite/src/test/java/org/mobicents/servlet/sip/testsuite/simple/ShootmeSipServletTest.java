...
//	private static final int TIMEOUT = 100000000;
    ...
    /*
	 * Non regression test for https://code.google.com/p/sipservlets/issues/detail?id=260
     */
    public void testShootmeEventParameterable() throws InterruptedException, SipException, ParseException, InvalidArgumentException {
        String fromName = "sender";
        String fromSipAddress = "sip-servlets.com";
        SipURI fromAddress = senderProtocolObjects.addressFactory.createSipURI(
                fromName, fromSipAddress);
        ...
        SipURI toAddress = senderProtocolObjects.addressFactory.createSipURI(
                toUser, toSipAddress);
        sender.sendSipRequest("INVITE", fromAddress, toAddress, null, null, false, new String[]{"Event"}, new String[]{"field-value;param1=paramValue1;param2=paramValue2"}, true);
        Thread.sleep(TIMEOUT);
        assertTrue(sender.isAckSent());
        assertTrue(sender.getOkToByeReceived());
    }
     ...
     * http://code.google.com/p/mobicents/issues/detail?id=2201
     * javax.servlet.sip.ServletParseException: Impossible to parse the
     * following header Remote-Party-ID as an address.
     */
    ...
    /**
     * non regression test for Issue 1104
     * http://code.google.com/p/mobicents/issues/detail?id=1104 Cannot find the
     * corresponding sip session to this subsequent request
     *
     * In conflict with Issue 1401
     * http://code.google.com/p/mobicents/issues/detail?id=1401 ACK request sent
     * by sip client after receiving 488/reINVITE is passed to sip application
     * Forbidden by SIP Spec "11.2.2 Receiving ACK" : "Applications are not
     * notified of incoming ACKs for non-2xx final responses to INVITE."
     *
     */
    ...
    public void testErrorResponseCodeForNonExistingDialog() throws InterruptedException, SipException, ParseException, InvalidArgumentException {
        ...
        Thread.sleep(100);
        assertEquals(481, sender.getFinalResponse().getStatusCode());
        sender.sendBye();
        Thread.sleep(100);
        assertTrue(!sender.getOkToByeReceived());
