 ...
 * 
 * <p>
 * The Application Router and the Container have a simple contract defined by the <code>SipApplicationRouter</code> interface. <br />
 ...
 * 
 * <ul>
 * <li>The properties file MUST be made available to the DAR and the location/content of this file MUST be accessible from a
 * hierarchical URI which itself is to be supplied as a system property "javax.servlet.sip.dar"</li>
 * <li>The propeties file has a simple format in which the name of the property is the SIP method and the value is a simple
 ...
 * 
 * </li>
 * <li>The properties file is first read by the DAR when the <code>init()</code> is first called on the DAR. The arguments
 * passed in the <code>init()</code> are ignored.</li>
 * 
 * <li>The properties file is refreshed each time <code>applicationDeployed()</code> or <code>applicationUndeployed()</code> is
 ...
 * <li>The identity of the subscriber that the DAR returns. It can return any header in the SIP request using the DAR directive
 * DAR:<i>SIP_HEADER</i> e.g "DAR:From" would return the sip uri in From header. Or alternatively it can return any string.</li>
 * 
 * <li>The routing region, one of the strings "ORIGINATING", "TERMINATING" or "NEUTRAL"</li>
 * <li>A SIP URI indicating the route as returned by the application route, it can be an empty string.</li>
 * <li>A route modifier which can be any one of the strings "ROUTE", "NO_ROUTE", or "CLEAR_ROUTE"</li>
 * <li>A string representing stateInfo. As stateInfo is for application router's internal use only, what goes in this is up to
 ...
 * the object <code>
 * SipApplicationRouterInfo</code> from the sip-router-info information in the properties file, starting from the first in the
